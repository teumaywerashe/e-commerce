...
function AdminHomePage() {
  ...
  return (
    <div className="p-8 max-w-screen-xl mx-auto">
      ...
      <h1 className="text-2xl font-light tracking-widest uppercase text-primary dark:text-white mb-10">Dashboard</h1>

      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-12">
        {stats.map((s) => (
          <div key={s.label} className="border border-neutral-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-6">
            <p className="text-[9px] font-semibold tracking-widest uppercase text-neutral-400 mb-3">{s.label}</p>
            <p className="text-3xl font-light text-primary dark:text-white mb-4">{s.value}</p>
            {s.link && (
              <Link to={s.link.to} className="text-[9px] font-semibold tracking-widest uppercase text-primary dark:text-white hover:opacity-60 transition-opacity">
                {s.link.text} →
              </Link>
            )}
          </div>
      ...
      <div className="border border-neutral-100 dark:border-gray-800 bg-white dark:bg-gray-900">
        <div className="px-6 py-4 border-b border-neutral-100 dark:border-gray-800 flex items-center justify-between">
          <p className="text-[10px] font-semibold tracking-widest uppercase text-primary dark:text-white">Recent Orders</p>
          <Link to="/admin/orders" className="text-[9px] font-semibold tracking-widest uppercase text-neutral-400 hover:text-primary dark:hover:text-white transition-colors">
            View All →
          </Link>
        </div>
            ...
            <tbody>
              {orders.length > 0 ? (
                orders.slice(0, 8).map((order) => (
                  <tr key={order._id} className="border-b border-neutral-50 dark:border-gray-800 hover:bg-neutral-50 dark:hover:bg-gray-800 transition-colors">
                    <td className="py-4 px-6 text-[10px] font-mono text-neutral-400">#{order._id.slice(-8)}</td>
                    <td className="py-4 px-6 text-[11px] font-medium text-primary dark:text-white">{order.user?.name}</td>
                    <td className="py-4 px-6 text-[11px] font-semibold text-primary dark:text-white">${order.totalPrice.toFixed(2)}</td>
