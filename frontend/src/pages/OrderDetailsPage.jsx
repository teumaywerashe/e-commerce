...
function OrderDetailsPage() {
  ...
  if (loading) return (
    <div className="max-w-3xl mx-auto px-6 py-16 animate-pulse space-y-4">
      <div className="bg-neutral-100 dark:bg-gray-800 h-4 w-32" />
      <div className="bg-neutral-100 dark:bg-gray-800 h-64 w-full" />
    </div>
  );
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950 py-16 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-12">
          <Link to="/my-orders" className="text-[10px] font-semibold tracking-widest uppercase text-neutral-400 hover:text-primary dark:hover:text-white transition-colors">
            ← Orders
          </Link>
          <span className="text-neutral-200 dark:text-gray-700">/</span>
        ...
        {orderDetails ? (
          <div className="border border-neutral-100 dark:border-gray-800">
            {/* Header */}
            <div className="px-8 py-6 border-b border-neutral-100 dark:border-gray-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <p className="text-[9px] font-semibold tracking-widest uppercase text-neutral-400 mb-1">Order</p>
                <p className="text-sm font-medium text-primary dark:text-white font-mono">#{orderDetails._id.slice(-12)}</p>
              ...
              <div className="flex gap-2">
                <span className={`text-[9px] font-semibold tracking-widest uppercase px-3 py-1.5 ${
                  orderDetails.isPaid ? "bg-primary dark:bg-white text-white dark:text-gray-900" : "bg-neutral-100 dark:bg-gray-800 text-neutral-600 dark:text-neutral-400"
                }`}>
                  {orderDetails.isPaid ? "Paid" : "Pending"}
                </span>
                <span className={`text-[9px] font-semibold tracking-widest uppercase px-3 py-1.5 ${
                  orderDetails.isDelevered ? "bg-primary dark:bg-white text-white dark:text-gray-900" : "bg-neutral-100 dark:bg-gray-800 text-neutral-600 dark:text-neutral-400"
                }`}>
            ...
            {/* Meta grid */}
            <div className="px-8 py-6 border-b border-neutral-100 dark:border-gray-800 grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <div key={label}>
                  ...
                  {lines.map((l, i) => (
                    <p key={i} className={`text-xs ${i === 0 ? "text-primary dark:text-white font-medium" : "text-neutral-500 dark:text-neutral-400"}`}>{l}</p>
                  ))}
                </div>
              ))}
            </div>
            ...
            <div className="px-8 py-6">
              ...
              <div className="space-y-4">
                {orderDetails.orderItems?.map((item, i) => (
                  <div key={i} className="flex items-center gap-5 py-4 border-b border-neutral-50 dark:border-gray-800 last:border-0">
                    <img src={item.image} alt={item.name} className="w-14 h-16 object-cover bg-neutral-50 dark:bg-gray-800 shrink-0" />
                    <div className="flex-1 min-w-0">
                      <Link to={`/product/${item.productID}`} className="text-[11px] font-medium text-primary dark:text-white hover:opacity-60 transition-opacity line-clamp-1">
                        {item.name}
                      </Link>
                      <p className="text-[10px] text-neutral-400 mt-0.5">Qty: {item.quantity}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-[11px] font-semibold text-primary dark:text-white">${(item.price * item.quantity).toFixed(2)}</p>
