...
function CartDrawer({ drawerOpen, setDrawerOpen, toggleCartDrawer }) {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-white z-50 flex flex-col transform transition-transform duration-300 ${drawerOpen ? "translate-x-0" : "translate-x-full"}`}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 h-16 border-b border-neutral-100">
          ...
          <button onClick={toggleCartDrawer} className="text-neutral-400 hover:text-primary transition-colors cursor-pointer">
        ...
        {cart?.products?.length > 0 && (
          <div className="px-6 py-6 border-t border-neutral-100">
            <div className="flex justify-between items-center mb-5">
              <span className="text-[11px] tracking-widest uppercase text-neutral-500">Total</span>
            ...
            <button onClick={goCheckout}
              className="w-full bg-primary text-white text-[11px] font-semibold tracking-widest2 uppercase py-4 hover:bg-neutral-800 transition-colors cursor-pointer">
              Checkout
            </button>
