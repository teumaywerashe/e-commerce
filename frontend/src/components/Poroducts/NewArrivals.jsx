...
function NewArrivals() {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/product/new-arrival`)
    ...
    <section className="py-20 bg-white dark:bg-gray-950">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-end justify-between mb-10">
          <div>
            <p className="text-[10px] font-medium tracking-widest3 uppercase text-neutral-400 mb-1">Just Dropped</p>
            <h2 className="text-xl font-light tracking-widest uppercase text-primary dark:text-white">New Arrivals</h2>
          </div>
            ...
            <div className="flex gap-2">
              {[["left", canScrollLeft], ["right", canScrollRight]].map(([dir, enabled]) => (
                <button key={dir} onClick={() => scroll(dir)} disabled={!enabled}
                  ...
                  {dir === "left" ? <FiChevronLeft className="h-3.5 w-3.5" /> : <FiChevronRight className="h-3.5 w-3.5" />}
                </button>
              ))}
            </div>
          )}
        ...
        <div ref={scrollRef} className="flex gap-4 overflow-x-auto no-scrollbar">
          {newArrivals.map((product) => (
            <Link key={product._id} to={`/product/${product._id}`} className="group shrink-0 w-[260px] md:w-[300px]">
              ...
              <p className="text-[11px] font-medium tracking-wide text-primary dark:text-white line-clamp-1 mb-1">{product.name}</p>
              <p className="text-[11px] text-neutral-500 dark:text-neutral-400">${product.price}</p>
            </Link>
          ))}
        </div>
      </div>
