      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="text-xl font-light tracking-widest uppercase text-primary">New Arrivals</h2>
          </div>
        ...
        <div ref={scrollRef} className="flex gap-4 overflow-x-auto no-scrollbar">
          {newArrivals.map((product) => (
            <Link key={product._id} to={`/product/${product._id}`} className="group shrink-0 w-[260px] md:w-[300px]">
              <div className="relative overflow-hidden bg-neutral-50 aspect-3/4 mb-3">
                <img src={product.images[0]?.url} alt={product.images[0]?.altText}
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                <span className="absolute top-3 left-3 bg-primary text-white text-[9px] font-semibold tracking-widest uppercase px-2 py-0.5">
                  New
                </span>
              </div>
              <p className="text-[11px] font-medium tracking-wide text-primary line-clamp-1 mb-1">{product.name}</p>
